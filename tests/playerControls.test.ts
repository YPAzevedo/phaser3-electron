...
import { JUMP_VELOCITY, MOVE_VELOCITY, computePlayerIntent } from "../src/game/playerControls";
...
describe("computePlayerIntent", () => {
  ...
  it("gives left priority when left and right are both held", () => {
    expect(computePlayerIntent({ left: true, right: true, up: false }, true)).toMatchObject({
      velocityX: -MOVE_VELOCITY,
      animation: "left",
    });
  });
  ...
  it("jumps when up is pressed and grounded", () => {
    expect(computePlayerIntent({ ...NO_INPUT, up: true }, true).jumpVelocity).toBe(JUMP_VELOCITY);
  });

  it("does not jump while airborne even if up is pressed", () => {
    expect(computePlayerIntent({ ...NO_INPUT, up: true }, false).jumpVelocity).toBeNull();
  });

  it("preserves horizontal movement while jumping", () => {
    const intent = computePlayerIntent({ left: false, right: true, up: true }, true);
